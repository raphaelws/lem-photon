...
//comentando pois a avalanche não importa por enquanto
 for (int i=0; i<nel; i++){
...
// TERMINA AQUI ----------------------------------------------------------------------
  ...
  std::ofstream sinalFile ("sinal.txt",std::ofstream::out | std::ofstream::trunc);
  //std::ofstream sinalFile ("sinal.txt",std::ofstream::app);

  for (int i = 0; i < nsBins; i++) {

  double wtl = sensor->GetSignal("wtlel", i) / ElementaryCharge;
  //sinal integrado
  sinalFile << i*bscale << " " <<  wtl << std::endl;
